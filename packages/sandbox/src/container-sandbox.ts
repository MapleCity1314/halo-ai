...
export class ContainerSandbox implements Sandbox {
  ...
  tools(): Record<string, ToolDefinition> {
    ...
    return {
      readFile: {
        ...
        execute: (a: Record<string, unknown>) => sb.readFile(String(a.path)),
      },
      writeFile: {
        ...
        execute: (a: Record<string, unknown>) =>
          sb.writeFile(String(a.path), String(a.content)).then(() => "File written."),
      },
      readdir: {
        ...
        execute: async (a: Record<string, unknown>) =>
          JSON.stringify(await sb.readdir(String(a.path))),
      },
      exec: {
        ...
        execute: async (a: Record<string, unknown>) => {
          const r = await sb.exec!(String(a.command), (a.args as string[]) ?? [], {
            cwd: a.cwd ? String(a.cwd) : undefined,
            timeout: a.timeout ? Number(a.timeout) : undefined,
          });
          return r.exitCode === 0 ? r.stdout : `exit=${r.exitCode}\n${r.stdout}\n${r.stderr}`;
        },
      },
      execShell: {
        ...
        execute: async (a: Record<string, unknown>) => {
          const r = await sb.execShell!(String(a.command), {
            allowShell: true,
            cwd: a.cwd ? String(a.cwd) : undefined,
            timeout: a.timeout ? Number(a.timeout) : undefined,
          });
  ...
  private _resolve(p: string): string {
    ...
    try {
      ...
      if (!n.startsWith(this._baseDir + path.sep) && n !== this._baseDir)
        throw new SandboxError(`Path traversal blocked: ${p}`, "EACCES");
      return resolved;
    }
    if (!realPath.startsWith(this._baseDir + path.sep) && realPath !== this._baseDir)
      throw new SandboxError(`Path traversal blocked: ${p}`, "EACCES");
    return realPath;
  }
}
